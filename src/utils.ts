 ...
 */
export function processKeyValueString(keyValueString: string, pairSeparator: string = ',', keyValueSeparator: string = '='): ({[key: string]: any}) {
  const result: {[key: string]: any} = {};


  function skipWhitespace(offset: number): number {// skip leading whitespace
  ...
  function parseValue(offset: number): [any, number] {
    ...
    // parse JSON
    if(keyValueString[offset] === '{' || keyValueString[offset] === '[' || keyValueString[offset] === '"' || keyValueString[offset] === "'") {
      const start = offset;
      const end = determineEndOfJson(offset);
    ...
    let end = keyValueString.indexOf(pairSeparator, offset);
    ...
    if(valueStringLower === 'false' || valueStringLower === 'disabled' || valueStringLower === 'off') return [false, end];
    if(valueStringLower === 'null' || valueStringLower === 'none' || valueStringLower === 'undefined') return [null, end];
    try {
  ...
  for(let i=0; i < keyValueString.length; ){
    ...
    if(keyEnd < 0 || keyEnd >= keyValueString.length){
      const key = keyValueString.substring(keyStart).trim();
      if(key.length > 0){
        result[key] = '';
      }
      break;
    }
    const key = keyValueString.substring(keyStart, keyEnd).trim();
